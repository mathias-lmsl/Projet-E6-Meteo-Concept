...
document.addEventListener('DOMContentLoaded', function () { //
    ...
    modeIcon.addEventListener('click', () => { // MODIFIÉ ICI pour cibler modeIcon directement
        body.classList.toggle('dark-mode'); // Alterne le thème
        const isDark = body.classList.contains('dark-mode'); // Vérifie mode
        ...
        // rafraichirCouleursGraphiques(); // MAJ couleurs - Cette fonction n'est pas définie dans ce fichier, commenter si non utilisée ici.
    });
    ...
    async function updateTable(tableName) { //
        ...
        document.querySelector('#enteteTab h2').textContent = tableName.charAt(0).toUpperCase() + tableName.slice(1) + 's'; // Met à jour le titre du tableau
        ...
        try {
            ...
            columns.forEach(col => { //
                // Pour la table utilisateur, ne pas afficher la colonne 'mdp'
                if (currentTable === 'utilisateur' && col.toLowerCase() === 'mdp') return; //

                const th = document.createElement('th'); //
                th.textContent = col; //
                th.classList.add(col.toLowerCase().replace(/\s+/g, '-')); // Remplace les espaces par des tirets pour les classes CSS //
                if (col.toLowerCase() === 'commentaire') th.classList.add('commentaire'); //

                // Bouton Ajouter
                // MODIFIÉ : Inclure 'chapelle' pour le bouton ajouter
                if (col === 'Actions' && ['carte', 'capteur', 'chapelle'].includes(tableName)) { //
            ...
            rows.forEach(row => { //
                ...
                columns.forEach(col => { //
                    ...
                    td.classList.add(col.toLowerCase().replace(/\s+/g, '-')); //
                    ...
                    if (col === 'Actions') { //
                        ...
                        const currentPkName = pkNameMap[currentTable]; //

                        if (currentPkName && row[currentPkName] !== undefined) { //
                            idValue = row[currentPkName]; //
                        } else { //
                            console.error("Impossible de déterminer la clé primaire pour la ligne:", row, "de la table:", currentTable); //
                            td.textContent = 'Erreur ID'; //
                            tr.appendChild(td); //
                            return; //
                        }

                        const rowDataForModal = { ...row }; //
                        delete rowDataForModal.Actions; //

                        // Bouton Modifier
                        const modifyBtn = createActionBtn('../img/modifier.svg', 'Modifier', () => //
                            openEditModal(rowDataForModal, currentTable, updateTable) //
                        );
                        td.appendChild(modifyBtn); //

                        // Bouton Supprimer (maintenant toujours ajouté si colonne Actions existe)
                        const deleteBtn = createActionBtn('../img/moins.svg', 'Supprimer', () => { //
                            if (confirm(`Êtes-vous sûr de vouloir supprimer cet élément de la table '${currentTable}' ?`)) { //
                                deleteRow(currentTable, idValue); //
                            }
                        });
                        ...
                        if (currentTable === 'utilisateur' && col.toLowerCase() === 'mdp') { //
                            td.textContent = '********'; //
                        } else { //
